...
##### FIX #7: Нет service layer для бизнес-логики
- [ ] ⚠️ Создать service layer
  - [ ] Вынести расчёт очков в `services/GameService.js`
  - [ ] Создать `calculatePoints()`, `determineWinner()`, `validateGameRules()`
  - [ ] Убрать бизнес-логику из API endpoints
  - **Проблема:** Бизнес-логика размазана по endpoints
  - **Решение:** См. CODE_REVIEW.md #7

##### FIX #8: Отсутствует DTO/Schema валидация
- [ ] ⚠️ Добавить Zod schemas
  - [ ] Создать `schemas/game.schema.js` с валидацией
  - [ ] Использовать runtime валидацию для всех inputs
  - [ ] Заменить ручную валидацию на Zod
  - **Проблема:** Нет единого места для описания структур
  - **Решение:** См. CODE_REVIEW.md #8
...
#### 🟡 P2: ЖЕЛАТЕЛЬНЫЕ УЛУЧШЕНИЯ (1-2 недели)

##### Архитектура
- [ ] FIX #9: Централизованная обработка ошибок
  - [ ] Создать `middleware/errorHandler.js`
  - [ ] Обработка ValidationError, NotFoundError, AuthError
  - [ ] Единый формат ошибок во всех endpoints
  - **Решение:** См. CODE_REVIEW.md #9

##### Производительность
- [ ] FIX #10: N+1 queries в некоторых endpoints
  - [ ] `api/day-stats.js` - возможно отдельные запросы для каждого дня
  - [ ] Использовать JOIN и GROUP BY
  - **Решение:** См. CODE_REVIEW.md #10

- [ ] FIX #11: Отсутствие пагинации в /api/all-games
  - [ ] При большом количестве игр endpoint вернёт всё
  - [ ] Добавить `page` и `limit` параметры
  - [ ] Максимум 100 записей за запрос
  - **Решение:** См. CODE_REVIEW.md #11

- [ ] FIX #12: Нет индексов в БД
  - [ ] CREATE INDEX на player_id, game_id, session_id
  - [ ] CREATE INDEX на date для быстрого поиска
  - **Решение:** См. CODE_REVIEW.md #12

##### Тестирование
- [ ] FIX #14: Нет интеграционных тестов
  - [ ] Создать `__tests__/integration/game-flow.test.js`
  - [ ] Тестировать полный flow: create game → calculate points → update rating
  - **Решение:** См. CODE_REVIEW.md #14
...
- [ ] FIX #21: Убрать console.log из production
  - [ ] Использовать logger с уровнями (debug/info/error)
  - [ ] См. CODE_REVIEW.md #21

- [ ] FIX #22: Убрать magic numbers
  - [ ] `86400` → `AUTH_TOKEN_TTL_SECONDS = 24 * 60 * 60`
  - [ ] Вынести в `constants/game.js`
  - [ ] См. CODE_REVIEW.md #22

- [ ] FIX #23: Убрать дублирование констант
  - [ ] Роли игроков в нескольких местах
  - [ ] Создать `constants/game.js` с ROLES
  - [ ] См. CODE_REVIEW.md #23
