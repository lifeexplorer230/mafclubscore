...
#### 🔴 P0: КРИТИЧЕСКИЕ ПРОБЛЕМЫ (исправить НЕМЕДЛЕННО - 1-2 дня)

##### FIX #1: Hardcoded admin token (CRITICAL)
- [ ] ⛔ Убрать hardcoded 'egor_admin' из `api/games/[id].js:30`
  - [ ] Удалить fallback значение
  - [ ] Выбрасывать ошибку 503 если ADMIN_AUTH_TOKEN отсутствует
  - [ ] Установить ADMIN_AUTH_TOKEN в Vercel Dashboard
  - [ ] Тестировать удаление игр с новым токеном
  - **Риск:** 🔴 CRITICAL - Любой может удалить игры
  - **Решение:** См. CRITICAL_FIXES.md FIX #1

##### FIX #2: Timing attacks в JWT (HIGH)
- [ ] 🔴 Заменить `!==` на `crypto.timingSafeEqual()`
  - [ ] `api/games/[id].js:32` - небезопасное сравнение токенов
  - [ ] Создать функцию `safeCompareTokens()` в `shared/middleware/auth.js`
  - [ ] Применить во всех местах сравнения токенов
  - **Риск:** 🟠 HIGH - Можно подобрать токен через timing
  - **Решение:** См. CRITICAL_FIXES.md FIX #2

##### FIX #3: SQL Injection в ORDER BY (HIGH)
- [ ] 🔴 Улучшить защиту от SQL injection
  - [ ] `shared/database.js:51` - regex не проверяет существование колонок
  - [ ] Создать `ALLOWED_COLUMNS` для каждой таблицы
  - [ ] Валидировать что колонка существует в таблице
  - **Риск:** 🟠 HIGH - Несуществующие колонки вызывают ошибки БД
  - **Решение:** См. CRITICAL_FIXES.md FIX #3

#### 🟠 P1: ВАЖНЫЕ ПРОБЛЕМЫ (исправить в течение 3-5 дней)

##### FIX #4: Failing tests с датами
- [ ] ⚠️ Исправить валидацию дат `__tests__/validation.test.js:74-75`
  - [ ] `validateDate('2025-02-30')` должен выбрасывать ошибку
  - [ ] JavaScript создаёт 2025-03-02 вместо ошибки
  - [ ] Добавить проверку что дата не изменилась после парсинга
  - **Проблема:** Тесты падают из-за неправильной валидации
  - **Решение:** См. CRITICAL_FIXES.md FIX #4
...
##### FIX #6: Отсутствие transactions в удалении игр
- [ ] ⚠️ Добавить transactions `api/games/[id].js:50-58`
  - [ ] Если второй DELETE упадёт, останутся orphaned game_results
  - [ ] Использовать `db.batch()` для атомарности
  - **Проблема:** Возможна несогласованность данных
  - **Решение:** См. CRITICAL_FIXES.md FIX #5
